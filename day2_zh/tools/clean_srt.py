#!/usr/bin/env python3
"""前處理：把 day2/ 的 SRT 統一解析、清洗填充語，輸出帶時間戳的 JSONL 中繼檔。
...
def merge(cues, gap=2.0, soft_limit=700, hard_limit=1400):
    """相鄰片段合併成段落：句尾標點結束且間隔夠大才斷段。

    Day 2 的 Atlas 上午有 43% 是全小寫無標點轉錄，永遠不滿足「句尾有標點」，
    整份會被合併成一段（實測有段落長達 6.4 萬字元）。故加上 hard_limit：
    超過就強制斷，優先挑時間間隔最大的位置，斷在講者換氣處而非句子中間。
    """
    paras = []
    cur = None
    for st, en, tx in cues:
        ...
        g = st - cur[1]
        if (g > gap and ends_sentence) or (len(cur[2]) > soft_limit and ends_sentence):
            paras.append(tuple(cur))
            cur = [st, en, tx]
        elif len(cur[2]) > hard_limit:
            # 無標點來源沒有句界可依，長度超標就斷；停頓較大處優先（讀起來較自然）
            paras.append(tuple(cur))
            cur = [st, en, tx]
        else:
            cur[1] = en
...
def main():
    ...
    if len(srts) != 6:
        print(f"警告：預期 6 份 srt，實際找到 {len(srts)} 份", file=sys.stderr)
