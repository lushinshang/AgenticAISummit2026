#!/usr/bin/env python3
"""產生給 LLM（Subagent 自身）對齊中英文、標註發言者與進行方案 B 段落合併的 Prompt。"""
import json
import re
import sys
...
def main():
    if len(sys.argv) < 3:
        print("Usage: process_chunk_b.py <stage> <chunk_name>", file=sys.stderr)
        print("Example: process_chunk_b.py atlas morning_s01_1.md", file=sys.stderr)
        sys.exit(1)
    ...
    prompt_lines = [
        ...
        "- 妳擅長處理包含專有名詞和中英日文夾雜的語句;對於不完全確定的詞彙,妳會選擇最可能的選項,並以括號附註其他候選。",
    ...
    prompt_lines.extend([
        ...
        "重要：只輸出處理後的 Markdown 譯文本體，不要輸出任何額外的前言或說明文字！",
    ])
