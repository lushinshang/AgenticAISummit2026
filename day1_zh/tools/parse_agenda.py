#!/usr/bin/env python3
"""解析 Day1Agenda.txt（已是台式繁中版）→ agenda.json，作為議程切分與譯名的權威來源。"""
import json
import re
from pathlib import Path

BASE = Path(__file__).resolve().parents[2]
SRC = BASE / "Day1Agenda.txt"
...
def split_persons(text: str):
    ...
    for part in re.split(r"[、,]\s*(?![^(（]*[)）])", text):
        ...
        if m:
            out.append({"name": m.group(1).strip(), "affiliation": m.group(2).strip()})
        else:
            out.append({"name": part, "affiliation": ""})
    return out
...
def main():
    ...
    key = {"Plenary Stage": "plenary", "Atlas Stage": "atlas",
           "Nexus Stage": "nexus", "Compass Stage": "compass"}
